...
// Variabel untuk mengatur pergerakan servo setiap 2 menit
unsigned long servoTimer = 0;
const long servoInterval = 120000; // 2 menit dalam milidetik
int servoPosition = 0; // Posisi awal servo
...
void setup() {
  ...
  // Inisialisasi servo
  myservo.attach(11);  // Hubungkan objek servo dengan pin 11
  myservo.write(servoPosition);  // Atur posisi awal servo
}
...
void loop() {
  ...
  if (dustDensity > 150) {
    ...
    // Menggerakkan servo setiap 2 menit ketika blower aktif
    unsigned long currentMillis = millis();
    if (currentMillis - servoTimer >= servoInterval && servoPosition < 180) {
      servoTimer = currentMillis;
      servoPosition += 20; // Tambahkan sudut 20 derajat
      myservo.write(servoPosition); // Gerakkan servo ke posisi baru
